...
import static damjei_SqlLogicas.SqlRepostarActualizarVehiculo.MANTENIMIENTO_OK;
...
public class SqlRevisiones {
    ...
    /**
     * obtenemos la clase a la que se quiere listar o una revision por su
     * idrevisiones
     *
     * @param variable
     * @return
    ...
    /**
     * Listamos las revisiones por el vehiculoid que tiene el esta_revision
     * false Asi se podra ver el si un vehiculo en concreto tiene revisiones
     * pendientes
     *
     * @param variable
     * @return
    ...
    public String ListarRevisionesVehiculo(String variable) throws SQLException {
        ...
        while (rs.next()) {
            ...
            if (!rs.getBoolean(ESTADO_REVISION)) {
                JsonObject jsonObject = new JsonObject();
        ...
        if (jsonArray.size() == 0) {//si no hay nada es quenotiene revisiones pendientes
            conexio = true;
            estadoRevision = true;
        ...
        } else {//mostramos las revisiones pendientes
            resposta = gson.toJson(jsonArray);
        }
