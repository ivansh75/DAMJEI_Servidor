    ...
    public String ObjetoEmpleados(String json) throws SQLException {
        ...
        if (claseJson != null && !claseJson.isJsonNull()) {
            ...
            if (empleatJson != null && !empleatJson.isJsonNull()) {
                ...
                if (request.getToken() == null) {//si no tenemos token no estamos registrados
                    // request.setAccion(LOGIN);
                    System.out.println("-----ESTAMOS EN PRUEBAS SIN CONTROL DE LOGIN-------HABILITAR LOGIN");
                    ...
                    if (numToken == -1 && request.getAccion() != 0) {
                        request.setAccion(LOGOUT);
                    }
                }
