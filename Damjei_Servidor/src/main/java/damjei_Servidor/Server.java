...
public class Server {
    ...
    public static void main(String[] args) {
        ...
        try {
            rn.initialize(host, puerto, logTime);
            rn.run();
        ...
        public synchronized void run() {
            try {
                ...
                while (running) {
                    ...
                    System.out.println("Consulta del cliente " + i);
                    
                    BufferedReader input = new BufferedReader(new InputStreamReader(client.getInputStream()));

                    String datos = input.readLine();
                    System.out.println("Consulta del cliente " + datos);
                    
                   
                    ClientRequest request  = gson.fromJson(datos, ClientRequest.class);
                    System.out.println("servidor reuest------"+ gson.toJson(request));
                    

                    if (logTime) {
                        //logFile.println(COMMENT + thisMoment() + "\n");
                        System.out.println(COMMENT + thisMoment() + "\n");
     ...
     */
    public void initialize(String host,int puerto, boolean logTime) throws IOException {
    ...
    private void replay(Socket client, String datos) throws SQLException {
        ...
        Thread t = new Thread(new Runnable() {
            ...
            public void run() {
                threadCount++;
                try {
                    String responseText = dataBase.ConsultaCliente(datos);
    ...
    protected void sendResponse(Socket cliente, String responde) throws IOException {
        ...
        System.out.println("Responde el Servidor-----------" + responde+ "\n");
